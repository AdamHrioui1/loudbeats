...
function Airpods() {
    ...
    useEffect(() => {
        window.document.title = 'LoudBeats Shop - Airpods'
    ...
    const nextBtn = () => {
        ...
        return counter === AirpodsData.length-1 ? setCounter(AirpodsData.length-1) : setCounter(counter+1)
    }
    ...
    return (
        <div className="App">
            <div className={`container ${activeMenu ? 'active' : ''}`} onTouchStart={e => startTouching(e)} onTouchEnd={e => endTouching(e)} onMouseMove={mouseMoving}>
                ...
                <div className="infos">
                    <span ref={companyAnimation}>{AirpodsData[counter].brand}</span>
                    <h1 ref={nameAnimation}>{AirpodsData[counter].name}</h1>
                    <p ref={detailsAnimation}>{AirpodsData[counter].description.length > 230 ? `${AirpodsData[counter].description.slice(0, 230)}...` : AirpodsData[counter].description}</p>
                    <Link to={`/singleairpod/${AirpodsData[counter].id}`} className='btn' onClick={seeDetails} onMouseEnter={() => setCursorIn(true)} onMouseOut={() => setCursorIn(false)}>See details</Link>
                </div>
                ...
                <div className="products_num">
                    ...
                    <span ref={totalCounter} >/{AirpodsData.length >= 10 ? AirpodsData.length : `0${AirpodsData.length}`}</span>
                </div>
            </div>
        </div>
    )
