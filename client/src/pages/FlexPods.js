...
function FlexPods() {
    ...
    useEffect(() => {
        window.document.title = 'LoudBeats Shop - Flexpods'
    ...
    const nextBtn = () => {
        ...
        return counter === FlexpodsData.length-1 ? setCounter(FlexpodsData.length-1) : setCounter(counter+1)
    }
    ...
    return (
        <div className="App">
            <div className={`container ${activeMenu ? 'active' : ''}`} onTouchStart={e => startTouching(e)} onTouchEnd={e => endTouching(e)} onMouseMove={mouseMoving}>
                ...
                <div className="infos">
                    <span ref={companyAnimation}>{FlexpodsData[counter].brand}</span>
                    <h1 ref={nameAnimation}>{FlexpodsData[counter].name}</h1>
                    <p ref={detailsAnimation}>{FlexpodsData[counter].description.length > 230 ? `${FlexpodsData[counter].description.slice(0, 230)}...` : FlexpodsData[counter].description}</p>
                    <Link to={`/singleflexpod/${FlexpodsData[counter].id}`} className='btn' onClick={seeDetails} onMouseEnter={() => setCursorIn(true)} onMouseOut={() => setCursorIn(false)}>See details</Link>
                </div>
                ...
                <div className={`imgs_container ${activateAnimation ? 'active' : ''}`} ref={imgsContainer} >
                    ...
                    {
                        FlexpodsData.map((p, i) => {
                            return (
                                <motion.img layoutId={p.imgs[0]} src={p.imgs[0]} alt={p.name} transition={{ duration: 1 }} style={{ transform: 'translate(-50%, -50%)'}} className={i === counter-1 ? 'past' : i === counter ? 'main_img' : i === counter+1 ? 'nd_img' : i === counter+2 ? 'rd_img' : 'none'} key={p.id} />
                            )
                        })
                    }
                </div>
                ...
                <div className="products_num">
                    ...
                    <span ref={totalCounter} >/{FlexpodsData.length >= 10 ? FlexpodsData.length : `0${FlexpodsData.length}`}</span>
                </div>
            </div>
        </div>
    )
